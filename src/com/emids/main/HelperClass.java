...
public class HelperClass {
	public static boolean supportingMethod(Scanner scanner, OnlineBookShop shop, Order order1, boolean condition) {
		while (condition) {
			System.out.println("please Enter your choice " + "\n" + "1)To view all books" + " \n"
					+ "2)to search books by bookname " + "\n" + "3)search by auther name" + "\n" + "4)add to cart"
					+ "\n" + "5)to order book directly" + "\n" + "6)Admin" + "\n" + "7)exit");
			int choice = scanner.nextInt();
			condition = methodofSwitch(shop, order1, condition, choice);
			System.out.println("\n");
		}
		return condition;
	}
	private static boolean methodofSwitch(OnlineBookShop shop, Order order1, boolean condition, int choice) {
		switch (choice) {
		case 1:
			shop.all();
			break;
